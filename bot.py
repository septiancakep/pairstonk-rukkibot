...
# Opsional: kirim semua notifikasi ke channel Telegram, bukan cuma ke chat
# personal yang /start. Isi dengan @namachannel (channel public) atau
# -100xxxxxxxxxx (channel private). Kosongkan kalau tidak dipakai.
# Bot HARUS dijadikan admin channel dengan izin "Post Messages" dulu.
...
async def check_new_pairs(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Target pengiriman: channel (kalau diset) + semua subscriber personal.
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    channel_line = (
        f"Channel tujuan: {TELEGRAM_CHANNEL_ID}\n" if TELEGRAM_CHANNEL_ID else ""
    )
    await update.message.reply_text(
        f"Status langganan personal: {'AKTIF ✅' if subscribed else 'nonaktif'}\n"
        f"{channel_line}"
        f"Total subscriber personal: {len(state['subscribers'])}\n"
